...
{
	[Desc("Projectile with customisable acceleration vector and parachute sequences.")]
	public class ParaBombInfo : IProjectileInfo
	{
		...
		[Desc("Parachute position relative to the projectile.")]
		public readonly WVec ParachuteOffset = new(0, 0, 384);

		[Desc("Enable parachute when projectile reaches this altitude or below.")]
		public readonly WDist ParachuteAltitude = new(1024);

		[Desc("Disable parachute when projectile reaches this altitude or below.")]
		public readonly WDist ParachuteCloseAltitude = new(256);
			...
			if (!world.FogObscures(pos))
			{
				...
				if (parachuteActive && parachuteAnim != null && !string.IsNullOrEmpty(info.ParachuteImage))
				{
					...
					if (parachutePaletteName != null && info.ParachuteIsPlayerPalette)
						parachutePaletteName += args.SourceActor.Owner.InternalName;

					var parachutePalette = wr.Palette(parachutePaletteName);
					var parachutePos = pos + info.ParachuteOffset;

					foreach (var r in parachuteAnim.Render(parachutePos, parachutePalette))
						yield return r;
				}
